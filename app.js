...
// convert to number
usersIntBalance = users.filter((user) => {
  return user.balance = Number(user.balance.replace(/\W/, "").replace(/\W/, ""));
});

let richUsers = usersIntBalance.filter(user => user.balance > 2000).map(user => user.phone);

console.log('Користувачі з балансом більше 2к:');
console.log(richUsers);

let totalBalance = 0;

usersIntBalance.forEach(user => {
